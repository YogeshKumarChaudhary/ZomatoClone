...
function DiningPage() {
  ...
  return (
    <div>
      ...
      <h1 className=" w-[1100px] mx-auto my-6 text-start font-semibold text-3xl pl-3 text-gray-800">
        Trending dining restaurants in Ahmedabad, Gujarat, India
      </h1>
      <div className=" flex pl-2 mt-5 mb-3 gap-3 w-[1100px] mx-auto">
        <div className="flex items-center gap-2 text-lg py-1 px-2 rounded-md cursor-pointer">
          {" "}
        ...
        <div
          ...
          onClick={() => setDiningRes(DiningResData)}
        >
          {" "}
          <span>
            <FcClearFilters />
      ...
      <div className=" w-[1100px] mx-auto flex flex-wrap gap-4 mb-[30px]">
        {DiningRes.length > 0 ? (
          DiningRes.map((res) => <DiningResCard data={res} key={res.id} />)
        ) : (
          <div>Dining Restaurants data not foud</div>
        )}
      </div>
      <div>
        <Accordian />
