...
function DeliveryPage() {
  ...
  const DeliveryResData = useSelector(
    (state) => state.Delivery.DeliveryResData
  );
  // console.log(ResData);
  useEffect(() => {
  ...
  return (
    <div className="w-[1100px] mx-auto">
      ...
      <h1 className="pl-2 text-start text-3xl font-semibold my-5">
        Delivery Restaurants in Ahmedabad, Gujarat, India
      </h1>
      <div className=" flex pl-2 mt-5 mb-3 gap-3">
        <div className="flex items-center text-lg gap-2  py-1 px-2 rounded-md cursor-pointer">
          {" "}
        ...
        <div
          className="flex items-center gap-2 border-2 py-1 px-2 rounded-md cursor-pointer"
        ...
        <div
          className="flex items-center gap-2  bg-slate-200 text-red-500   font-semibold border-2 py-1 px-2 rounded-md cursor-pointer"
          onClick={() => setResData(DeliveryResData)}
        >
          {" "}
          <span>
            <FcClearFilters />
      ...
      <div className="flex flex-wrap gap-4 mb-[30px]">
        {ResData.length > 0 ? (
          ResData.map((res) => <ResCard key={res.id} data={res} />)
        ) : (
          <div>Delivery Restaurants data not found</div>
        )}
      </div>
      <Accordian />
    </div>
