  ...
  return (
    <div className="">
      {showSearchContainer && (
        <div>
          {products.length && (
            <div className="h-[500px] py-6 rounded-lg w-[500px] overflow-scroll absolute  z-20 top-[330px] left-[650px] bg-white">
              <div
                onClick={() => setShowSearchContainer(false)}
                className="  cursor-pointer absolute right-4 text-3xl bg-slate-50 p-1 rounded-xl top-2 "
              >
                <IoMdClose />
              </div>
              {products.map((item, index) => (
                <div className="flex gap-5 mt-4 item-center pl-3 " key={index}>
                  <div>
                    <img
