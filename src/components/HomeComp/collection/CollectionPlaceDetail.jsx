    ...
    <div className=" w-[1100px] mx-auto">
      ...
      <div className="flex gap-1 my-4 transition-all duration-300">
        ...
        <Link to={"/Ahmedabad/order-food-online"}>
          {" "}
          <p className=" hover:text-red-500">Ahmedabad /</p>
        </Link>
        <Link to={"/"}>
          {" "}
          <p className=" hover:text-red-500">collectiones /</p>
        </Link>
        <p className=" text-gray-500">{placeInfo.name}</p>
      </div>
      <div
        style={{ backgroundImage: `url(${placeInfo.image})` }}
        className="w-[1100px] mx-auto bg-cover bg-center bg-no-repeat h-[320px] text-white  relative text-start rounded-lg "
      >
      ...
      <div className="flex flex-wrap w-[1100px] mx-auto mt-10 gap-5">
        {placeInfo.restaurant && placeInfo?.restaurant.length > 0 ? (
