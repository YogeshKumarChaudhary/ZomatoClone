              ...
              {data.map((item) => (
                <div key={item.id} className="border-2  rounded-lg">
                  <div
                    ...
                    onClick={() => questionClickHandler(item.id)}
                  >
                    <p className=" text-xl">{item.question}</p>
                    <span>
                      {isquestion === item.id && <IoIosArrowUp />}
                      {isquestion !== item.id && <FaAngleDown />}
                    </span>
                  </div>

                  {isquestion === item.id && (
                    <div>
                      <p className=" text-start px-5 text-lg text-gray-500 py-2">
                        {item.answer}
                      </p>
                    </div>
                  )}
